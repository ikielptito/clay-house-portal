// ONE-TIME USE — patches the latest report's workPackages and recalculates percentages.
...
function weightedPct(wp, items) {
  ...
  for (const item of items) {
    ...
    weightedSum += (parseFloat(wp?.[item] ?? 0) || 0) * w;
    totalWeight += w;
  }
...
module.exports = async function handler(req, res) {
  ...
  // Fetch latest index entry
  const raw = await redis.lrange('report:index', 0, 0);
  if (!raw?.length) return res.status(404).json({ error: 'No reports found' });

  const indexEntry = typeof raw[0] === 'string' ? JSON.parse(raw[0]) : raw[0];
  const { id, period } = indexEntry;

  // Fetch full report
  const report = await redis.get(`report:${id}`);
  if (!report) return res.status(404).json({ error: 'Report not found' });

  // Patch workPackages, keep everything else
  const patched = { ...report, workPackages };
  await redis.set(`report:${id}`, patched);

  // Recalculate and update index entry
  const cPct = weightedPct(workPackages, BUILDING_ITEMS);
  const oPct = weightedPct(workPackages, ALL_ITEMS);

  await redis.lset('report:index', 0, JSON.stringify({ id, period, cPct, oPct }));

  return res.json({ success: true, id, period, cPct, oPct, workPackages });
};
